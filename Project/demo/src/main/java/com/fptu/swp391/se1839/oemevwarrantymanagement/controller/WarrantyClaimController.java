...
public class WarrantyClaimController {
        ...
        @PutMapping("/claims/{id}/assign-tech")
        public ResponseEntity<ApiResponse<ChooseTechnicalResponse>> chooseTechnician(
                        @PathVariable("id") Long claimId,
                        @RequestBody ChooseTechnicalRequest request) {
                ChooseTechnicalResponse ctr = this.warrantyClaimService.handleChooseTechnical(claimId, request);
        ...
        @PutMapping(value = "/claims/{id}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
        @PreAuthorize("hasAnyAuthority('ADMIN','TECHNICIAN')")
        public ResponseEntity<ApiResponse<UpdateClaimResponse>> updateClaim(
                        @RequestPart("claim") UpdateClaimRequest request,
                        @PathVariable("id") long claimId,
                ...
                Long userId = Long.parseLong(jwt.getClaim("userId").toString());
                UpdateClaimResponse response = this.warrantyClaimService.handleUpdateClaim(request, claimId,
                                serviceCenterId,
                                attachments, userId);

                var result = ApiResponse.<UpdateClaimResponse>builder()
                                .status(HttpStatus.CREATED.toString())
                                .message("Create claim successfully")
                                .data(response)
                                .build();
